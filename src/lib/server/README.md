...
| `GET /api/lookup?term=...&kind=movie\|series` | Reads only matching instances' metadata lookup endpoints and returns `CatalogResponse` (metadata plus existing instance IDs); it does not fetch library, queue, profile, or episode enrichment. Request cancellation aborts all upstream lookups, including response-body reads. One instance's failure does not hide another's results. As a convenience, omitting `kind` searches both kinds. Empty live searches return `[]`, never live trending claims. |
...
| `DELETE /api/media` | Requires `{instanceId, remoteId, kind, deleteFiles}` with an explicit boolean. Verifies the upstream identity and removes only that instance's movie/series entry, optionally deleting its files. |
| `DELETE /api/episodes` | Requires `{instanceId, remoteId}` and exactly one of `episodeId` or `seasonNumber` (including specials, `0`). Resolves current Sonarr files, validates ownership before deletion, and deduplicates shared files. Leaves the series and monitoring intact. Partial failures report confirmed deletions; affected snapshots refresh even after failed writes. |
